...
export const getLocations = async () => {
  const response = await axios.post(API_CONFIG.LOCATIONS_URL, {
...
export const getWeatherDataByLocation = async (
  ...
  const geoResponse = await axios.get(
    `${API_CONFIG.GEOCODING_URL}?name=${encodeURIComponent(
      location
    )}&country=IN`
  );
  ...
  if (startDate === endDate) {
    //if this returns turue, the user has selected a single day
